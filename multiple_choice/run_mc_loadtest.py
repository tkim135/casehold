...
def main():
    ...
    model = GPT2ForMultipleChoice.from_pretrained(
        pretrained_model_name_or_path=custom_args.weight,
        config=config,
    )
    model.named_parameters()['transformer.wte.weight'] = model.named_parameters()['transformer.wte.weight'][:50257,:]
    ...
    if model_args.model_name_or_path == 'gpt2-xl':
        for i in range(48):
            for tensor_name in tensor_names:
                ...
                model.named_parameters()[full_tensor_name] = torch.transpose(model.named_parameters()[full_tensor_name], 0, 1)
